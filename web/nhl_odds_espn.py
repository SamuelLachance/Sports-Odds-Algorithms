"""Build NHL open/close moneylines + totals from ESPN's core odds API (median consensus)."""
...
def _provider_line_nhl(item: dict[str, Any]) -> dict[str, float | None]:
    """One book's home-oriented NHL lines: open/close ML, puck line, totals."""
...
def collect_day_rows(day: date, *, use_cache: bool = True) -> list[dict[str, Any]]:
    ...
    if use_cache and cache_file.is_file():
        try:
            ...
            # Empty / partial-day / all-failed / pre-open-ml caches must not stick.
            if (
                cached
    ...
    def _odds_row(event: dict[str, Any]) -> dict[str, Any]:
        ...
        away_spread = consensus.get("away_close_spread")
        close_total = consensus.get("close_total")
        # Keep puck-line-only / totals-only / away-only rows — do not require a ML.
        if (
            home_ml is None
            and away_ml is None
            and home_spread is None
            and away_spread is None
            and close_total is None
        ):
            return row
        row.update(
            {
