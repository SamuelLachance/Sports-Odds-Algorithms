"""Live WNBA v2 inference: artifact loading + current-season replay + prediction.
...
def wnba_season_for_date(target: date_cls | str) -> int:
    """WNBA season = calendar year (season runs May-October)."""
...
def _fetch_events_cached(season: int, *, current: bool) -> tuple[list[dict[str, Any]], bool]:
    ...
    try:
        ...
        # Hard fail with no soft cache: mark stale so history fallback is honest.
        return [], True
...
def _history_season(season: int) -> list[dict[str, Any]] | None:
    ...
    season_dir = PROJECT_ROOT / ".build-cache" / "wnba-history" / str(season)
...
def get_live_context(day_iso: str) -> dict[str, Any] | None:
    ...
    season = wnba_season_for_date(target)
...
def predict_matchup_v2(
    ...
    *,
    ...
    engine: WnbaFeatureEngine = context["engine"]
