...
def _fetch_json(url: str, timeout: int = 20) -> dict[str, Any]:
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "Sports-Odds-Algorithms/1.0"},
    )
...
def fetch_scoreboard(
    ...
    """Return scheduled games for a league on a date (default: today)."""
    ...
    for offset in range(0, days_ahead + 1):
        dates_to_check.append(base.fromordinal(base.toordinal() + offset))
    ...
    for check_date in dates_to_check:
        ...
        url = (
            ...
            f"{config['sport_path']}/scoreboard?dates={date_param}"
        )
        try:
    ...
    if not games and on_date is None:
        url = f"https://site.api.espn.com/apis/site/v2/sports/{config['sport_path']}/scoreboard"
        try:
            payload = _fetch_json(url)
        except urllib.error.URLError:
            return games
...
def fetch_team_schedule(league: str, espn_team_id: str, season: int) -> list[dict[str, Any]]:
    ...
    url = (
        f"https://site.api.espn.com/apis/site/v2/sports/"
        f"{config['sport_path']}/teams/{espn_team_id}/schedule?season={season}"
    )
    try:
        payload = _fetch_json(url)
    except urllib.error.URLError:
