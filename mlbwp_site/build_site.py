...
.valstrip{display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin:14px 0 2px;padding:12px 14px;
  border:1px solid rgba(232,178,58,.5);border-radius:12px;
  background:linear-gradient(90deg,rgba(232,178,58,.14),rgba(232,178,58,.04))}
.valstrip .vh{font-family:var(--mono);font-size:10px;letter-spacing:.12em;font-weight:700;
  background:#e8b23a;color:#181200;padding:2px 8px;border-radius:4px}
.valstrip .vs{display:flex;align-items:baseline;gap:8px;padding:7px 12px;border:1px solid var(--line-2);
  border-radius:10px;background:var(--panel);color:var(--ink);text-decoration:none;font-size:13px}
.valstrip .vs:hover{border-color:#e8b23a}
.valstrip .vs b{font-family:var(--mono)}
.valstrip .vs .ev{font-family:var(--mono);font-weight:700;color:#d99a1e}
.valstrip .vs .vodds{font-family:var(--mono);color:var(--muted);font-size:11.5px}
.valstrip .vs .when{font-size:11px;color:var(--muted)}
.valstrip .vnote{flex-basis:100%;font-size:11px;color:var(--muted);margin-top:2px}
...
.odds{font-family:var(--mono);font-size:10.5px;color:var(--faint);margin-right:7px;font-variant-numeric:tabular-nums}
...
/* ---- track record ---- */
.rechero{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:14px}
@media(max-width:520px){.rechero{grid-template-columns:1fr}}
.rechero .rh{background:var(--panel);border:1px solid var(--line);border-radius:12px;padding:14px 16px}
.rechero .rhk{display:block;font-size:9.5px;letter-spacing:.07em;text-transform:uppercase;color:var(--muted)}
.rechero .rhv{display:block;font-family:var(--mono);font-size:38px;font-weight:700;line-height:1.1;margin-top:4px}
.rechero .rhv i{font-style:normal;font-size:20px;font-weight:600;color:var(--muted);margin-left:1px}
.rechero .rhs{display:block;font-size:11px;color:var(--muted);margin-top:3px}
.rechero .rhs.pos,.rechero .rhv.pos{color:var(--fav)}
.rechero .rhs.neg,.rechero .rhv.neg{color:var(--warn)}
.chip{display:inline-block;font-family:var(--sans);font-weight:700;font-size:10px;letter-spacing:.04em;
  padding:2px 8px;border-radius:999px;border:1px solid var(--line-2);color:var(--muted);background:var(--sunk)}
...
.cbar i{display:block;height:100%;background:var(--accent);border-radius:3px}
tr.recrow{cursor:default}
tr.recrow[onclick]{cursor:pointer}
...
function nflGamePage(key){
  ...
  const LBL={elo:"Team strength (Elo)", qb:"Quarterback", units:"Pass/run units (EPA)",
             ...
             ts:"Player ratings (11v11 TS)", luck:"Fumble luck", abs:"Injuries / absences"};
...
function recUnitsPanel(lg){
  const head=`<h3>Units <span class="sub">flat 1u per pick &middot; pre-game consensus price</span></h3>`;
  const u=recUnits(lg);
  if(!u) return `<div class="panel" style="margin-top:16px">${head}
    <div class="empty">No priced picks settled yet &mdash; units start accruing once pre-game prices are
      archived${lg==="mlb"?"":" (MLB first)"}.</div></div>`;
  const p=u.pick;
  const box=(k,v,cls)=>`<div class="b"><div class="k">${k}</div><div class="v${cls?" "+cls:""}">${v}</div></div>`;
  const tiles=p?[
    box("Record",`${p.w}&ndash;${p.l}`),
    box("Units net",fmtU(p.net),p.net>=0?"pos":"neg"),
    box("ROI",(p.roi>=0?"+":"&minus;")+Math.abs(p.roi*100).toFixed(1)+"%",p.roi>=0?"pos":"neg"),
    box("Staked",p.staked.toFixed(0)+"u"),
  ...
  const split=`<div class="twrap" style="margin-top:10px"><table>
  ...
  const monthTbl=months?`<div class="twrap" style="margin-top:10px"><table>
    <thead><tr><th>Month</th><th>Bets</th><th>Units</th><th>Cum units</th></tr></thead>
    <tbody>${months}</tbody></table></div>`:"";
  const unpriced=u.n_unpriced?` ${u.n_unpriced} graded pick${u.n_unpriced===1?"":"s"} had no archived price
    (tracking started ${u.priced_since?`on ${u.priced_since}`:"later than pick tracking"}) &mdash; excluded, never back-filled.`:"";
  const thin=p&&p.n<30?`<div class="sub" style="margin-top:8px"><b>n=${p.n} &mdash; not yet meaningful.</b>
    Units swing hard at this sample; treat it as the accounting existing, not evidence of an edge.</div>`:"";
  return `<div class="panel" style="margin-top:16px">${head}
    ${p?`<div class="statgrid" style="margin-top:10px;grid-template-columns:repeat(auto-fit,minmax(110px,1fr))">${tiles}</div>`:`<div class="empty">Priced picks are recorded but none has settled yet.</div>`}
    ${recUnitsChart(u)}
    ${thin}
    <details class="sched" style="margin-top:10px"><summary>Breakdown &amp; method</summary><div class="body">
      ${split}
      ${monthTbl}
      <div class="sub" style="margin-top:10px">1 unit on the model's pick at the <b>last strictly-pre-game</b>
        consensus price (median across US books, archived before first pitch). A win banks
        price&nbsp;&minus;&nbsp;1, a loss costs 1. Headline covers picks only; leans and EARLY are settled
        beside it, never inside.${unpriced}</div>
    </div></details></div>`;
}
