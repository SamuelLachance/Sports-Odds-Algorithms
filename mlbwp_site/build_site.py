...
function gcard(g){
  ...
  const spA=g.away_sp==="TBD"?`<span class="tbd">TBD</span>`:`${g.away_sp} ${pitEra(g.away_sp)}`;
  const spH=g.home_sp==="TBD"?`<span class="tbd">TBD</span>`:`${g.home_sp} ${pitEra(g.home_sp)}`;
  return `<a class="gc" href="#/game/${g.game_pk}" data-card="${g.game_pk}"
    ...
    <div class="edge"><span class="k">edge</span>
      ...
      ${e.baserun!=null&&Math.abs(e.baserun)>=0.3?`<span class="${e.baserun>=0?"p":"n"}">run ${sgn(e.baserun)}</span>`:""}
      ${g.lineup_source==="official"?'<span class="k" style="color:var(--fav)">&middot; lineups in</span>'
        :g.lineup_source==="projected"?'<span class="k" style="color:#d9a441">&middot; proj lineup</span>'
        :(g.pitcher_known?"":'<span class="k">&middot; starters tbd</span>')}</div>
  </a>`;
}
function board(){
        ...
        <div class="panel"><h3>Our projection</h3>
          ...
          <div class="sub" style="margin-top:8px;color:var(--faint)">
            ${g.lineup_source==="official"
              ...
              ? "Projected lineup (each team's most recent) &mdash; the full model runs on the likely nine and upgrades automatically when the official lineup posts."
