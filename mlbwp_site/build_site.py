...
function nflPage(){
  ...
  const ncard=g=>{
    const hp=g.ph, homeWin=hp>=0.5, done=g.hs!=null;
    const pick=homeWin?g.home:g.away, pp=Math.max(hp,1-hp);
    let badge=`<span class="lbadge proj">Week ${g.w}</span>`;
    ...
    return `<a class="gc" href="#/season/${g.w}">
      <div class="top"><span class="lv">${done?`FINAL ${g.as}-${g.hs}`:`${g.d.slice(5)} &middot; ${g.t?g.t.slice(0,5):""} ET`}</span>
        ${badge}
