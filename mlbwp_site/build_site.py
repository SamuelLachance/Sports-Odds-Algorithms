...
async function boot(){
  try{
    ...
    route();
    pollLive(true);                 // first poll always runs, even if the tab loads hidden
    setInterval(pollLive, 25000);   // MLB Stats API caches ~20s; poll a touch slower
...
function nflPage(){
  ...
  const wk1=sch.filter(g=>g.w===1).map(g=>{const pick=g.ph>=0.5?g.home:g.away,pp=Math.round(100*Math.max(g.ph,1-g.ph));
    return `<tr onclick="location.hash='#/season/1'"><td class="sub">${g.d.slice(5)}</td>
      <td class="a">${g.away} <span class="sub">at</span> ${g.home}</td>
      <td><span class="pill ${pp>=65?"strong":""}">${pick} ${pp}%</span></td></tr>`;}).join("");
  ...
  $("#view").innerHTML=`<div class="controls"><div class="rail">${rail}</div></div>
    ...
    ${sch.length?`<div class="grid" style="margin-bottom:4px">
      <div class="panel"><h3>Week 1 picks &middot; <a href="#/season" style="color:var(--accent)">full season &rarr;</a></h3>
        <div class="twrap"><table><tbody>${wk1}</tbody></table></div></div>
...
function nflSeason(wk){
  ...
  const row=g=>{
    const pick=g.ph>=0.5?g.home:g.away, pp=Math.round(100*Math.max(g.ph,1-g.ph)), done=g.hs!=null;
    ...
    return `<tr><td class="sub">${g.d.slice(5)} ${g.t?g.t.slice(0,5):""}</td>
      ...
      <td><div class="pbar" style="min-width:110px;margin:0"><div class="h" style="width:${Math.round(g.ph*100)}%"></div><div class="mid"></div></div></td>
      ${res}</tr>`;};
  $("#view").innerHTML=`<div class="eyebrow">NFL &middot; ${n.season} season</div>
    ...
    <div class="sub" style="margin-bottom:12px">Probability bar shows the <b>home</b> side. QB1s from live depth charts; predictions and the standings simulation refresh on every data build.</div>
...
function nflTeamPage(code){
  ...
  const srow2=g=>{const homeG=g.home===code, opp=homeG?g.away:g.home, pw=homeG?g.ph:1-g.ph;
    const pick=g.ph>=0.5?g.home:g.away;
    return `<tr><td><span class="num">W${g.w}</span></td><td class="sub">${g.d.slice(5)}</td>
      ...
      <td><span class="pill ${Math.max(g.ph,1-g.ph)>=0.65?"strong":""}">${pick}</span></td></tr>`;};
