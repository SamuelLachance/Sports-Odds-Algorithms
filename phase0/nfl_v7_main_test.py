"""MAIN-PROTOCOL TEST 3: v7 (cohort salary prior) column vs v6: with v6 in the model (col 7), is the OLD rating system
...
X15 = X14.copy(); X15[:, 7] = np.load("data/nfl_v7_feature.npy")
...
print(f"\nmain TEST (n={len(yt)}):")
print(f"  current model (v6 column)     LL {ll_base:.5f}")
print(f"  v7 column (cohort salary)         LL {ll_drop:.5f}  "
      f"(delta {float(d.mean()):+.5f}  CI [{lo:+.5f}, {hi:+.5f}])")
print(f"\n-> {'ADOPT v7 column' if ll_drop < ll_base else 'keep v6 column'}")
json.dump({"base_ll": round(ll_base, 5), "drop_rq_ll": round(ll_drop, 5),
          ...
          open("data/nfl_v7_main_test.json", "w"), indent=1)
