...
def record_block(ledger_path: Path = PRED_LEDGER, cap: int = RECORD_CAP) -> dict:
    ...
    UNITS ARE NOT HERE (decision 2026-08-12): units track the EDGE-badge bets
    and nothing else. Picks are scored on accuracy/log loss only — they carry
    no stake. The badge bets' P&L block is board.record.bets, produced by
    market/edge_ledger.site_block() off the ledger rows (the actual bet
    slips). The pre-game consensus prices archived on ledger entries
    (hdec/adec, mlbwp/pred_ledger.py) remain archived — data, not accounting.
    """
    import math
