"""Train CFB v2 win-probability + margin models with walk-forward evaluation.

Outputs to data/models/cfb_v2/:
...
TABLE_PATH = PROJECT_ROOT / "data" / "cfb_history" / "training_table.csv"
ODDS_CSV = PROJECT_ROOT / "data" / "supplemental" / "closing-odds" / "cfb.csv"
OUT_DIR = PROJECT_ROOT / "data" / "models" / "cfb_v2"

# With current cfb.csv (min season 2022), 2022 warms Elo/EWMAs; first OOS is 2023.
# After earlier backfills, lower FIRST_EVAL_SEASON if emit_from drops below 2022.
FIRST_EVAL_SEASON = 2023
ELO_BASELINE_MAE = 14.15
...
# Market-aware heads: closing odds are dense enough (~92% ML, ~98% spread).
CLF_MARKET_FEATURES = ("mkt_home_prob", "has_market")
MARGIN_MARKET_FEATURES = ("mkt_home_spread", "has_spread")
MIN_MARKET_ROWS = 200

# CFB season periods: early (Aug-Sep), mid (Oct), late+bowl (Nov-Jan)
...
def attach_market(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # Elo-implied home win from feature elo_diff (already includes HFA when not neutral)
    frame["elo_home_prob"] = frame["elo_diff"].map(
        lambda d: elo_to_prob(float(d), z=400.0)
    )
    ...
    for eval_season in range(FIRST_EVAL_SEASON, end_season + 1):
        for period in (0, 1, 2):
            ...
            if train.empty or test.empty:
                continue
            # Need some prior-season history before first eval chunk
...
def train_final_artifacts(frame: pd.DataFrame, oos: pd.DataFrame, end_season: int) -> dict:
    ...
    if not train_market_clf:
        print(
            f"skipping CFB market clf heads: only {n_odds} rows with odds "
    ...
    if train_market_clf:
        ...
        # OOS walk-forward frame is label-only; join features from the attached table.
    ...
    # Engine snapshots: replay CSV through end of snapshot seasons
    odds = pd.read_csv(ODDS_CSV)
    from web.cfb_v2.feature_engine import cfb_season_of

    odds["day"] = pd.to_datetime(odds.date).dt.date
    odds["season"] = odds.day.map(cfb_season_of)
    all_games = csv_rows_to_games(odds.to_dict(orient="records"))

    snapshot_seasons = sorted({int(s) for s in odds.season.unique() if int(s) < end_season})
    # Keep last two completed seasons as snapshots
    ...
    close_mae = float(
        np.mean(np.abs(oos.dropna(subset=["home_close_spread"]).margin
                       + oos.dropna(subset=["home_close_spread"]).home_close_spread))
    )
    ...
    metadata = {
        ...
        "oos_close_spread_mae": round(close_mae, 4),
        "margin_sigma": round(margin_sigma, 4),
        "ship_models": ship_models,
        "player_proxy_note": (
            "No roster/QB box data; features close_win_ewma, blowout_*, margin_vol, "
            "scoring_vol, one_score_rate are score-derived luck/volatility proxies."
        ),
    ...
    # Rename columns for backtest_cfb_bets compatibility
    export = oos.rename(
        columns={
            "home_close_spread": "home_spread",
            "home_open_spread": "home_spread_open",
