...
from web.ensemble_ml.config import TRAIN_LEAGUES, get_dataset_profile, metadata_path  # noqa: E402
...
def build_report(cutoff: str) -> dict:
    ...
    for league in sorted(TRAIN_LEAGUES):
        ...
        row = {
            ...
            "ensemble_market_logloss": (
                ensemble.get("market_holdout_logloss")
                or ensemble.get("all_market_logloss")
                if ensemble
                else None
            ),
...
def _print_table(report: dict) -> None:
    print(
        ...
        f"{'Ens Rows':>9} {'Ens LL':>8} {'Mkt LL':>8}"
    )
    print("-" * 82)
    for row in report["leagues"]:
        print(
            ...
            f"{row['ensemble_market_logloss'] or '—':>8}"
        )
