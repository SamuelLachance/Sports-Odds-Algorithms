"""Download Torvik game_factors CSVs (public toRvik-data) for CBB four-factor history.

Source: https://github.com/andreweatherman/toRvik-data/tree/main/game_factors
Coverage: seasons ending 2008–2023 (no 2024+ archive yet).

Usage:
  python scripts/fetch_torvik_game_factors.py
  python scripts/fetch_torvik_game_factors.py --start-year 2018 --end-year 2023
"""
...
OUT_DIR = PROJECT_ROOT / "data" / "supplemental" / "torvik-game-factors"
...
def fetch_year(year: int, *, force: bool = False) -> Path | None:
    ...
    dest = OUT_DIR / f"game_factors_{year}.csv"
    if dest.is_file() and dest.stat().st_size > 100_000 and not force:
        ...
        return dest
    url = f"{BASE}/game_factors_{year}.csv"
