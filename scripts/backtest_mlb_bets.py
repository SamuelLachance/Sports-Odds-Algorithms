"""Backtest MLB v2 moneyline bet policy on walk-forward OOS predictions.

Simulates betting vs the closing line (conservative: closing is the sharpest
price) across an edge-threshold grid, then reports per-season ROI, drawdown,
and a robust recommended policy. Also simulates at the opening line as an
optimistic bracket (live bets are placed in between).
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest MLB v2 bet policy")
    ...
    print(
        ...
        f"({open_frame.season.min()}-{open_frame.season.max()})\n"
    )

    # Live bets are recorded in the morning (near the open), so rank policies on
    # opening-line performance; report the closing-line result as a floor.
    grid_results: list[tuple[dict, dict, dict]] = []
    for min_edge in (1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 6.0):
        for min_ev in (0.0, 2.0, 4.0):
            for ml_lo, ml_hi in ((-350, 300), (-200, 200), (-150, 250)):
                params = {
    ...
    for params, open_result, close_result in grid_results[:10]:
        print(
