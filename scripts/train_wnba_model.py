"""Train WNBA v2 win-probability + margin models with walk-forward evaluation.

Outputs to data/models/wnba_v2/:
...
from scripts.build_wnba_training_table import load_season  # noqa: E402
from web.wnba_v2.feature_engine import FEATURE_COLUMNS, WnbaFeatureEngine  # noqa: E402
from web.wnba_v2.replay import replay_season  # noqa: E402
...
def _month_periods_key(frame: pd.DataFrame) -> pd.Series:
    """WNBA season months: May-Jun = 0, Jul = 1, Aug-Oct = 2."""
...
def train_final_artifacts(frame: pd.DataFrame, oos: pd.DataFrame, end_season: int) -> None:
    ...
    for season in range(1997, max(snapshot_seasons) + 1):
        games, _odds = load_season(season)
        if not games:
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train WNBA v2 model")
